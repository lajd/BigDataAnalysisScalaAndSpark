...
object WikipediaRanking extends WikipediaRankingInterface:
  ...
  val appName = "wikipedia-word-count"
  val master = "local"
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int =
    rdd.aggregate(0)((count, article) => (if (article.mentionsLanguage(lang)) (count + 1) else (count + 0)), _ + _)
  ...
  def rankLangs(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] =
    langs.map(
      lang => Tuple2(lang, occurrencesOfLang(lang, rdd))
    ).sortBy(_._2).reverse
  ...
  def makeIndex(langs: List[String], rdd: RDD[WikipediaArticle]): RDD[(String, Iterable[WikipediaArticle])] =
    rdd.flatMap(
      wikiArticle => langs.map(
        l => if (wikiArticle.mentionsLanguage(l)) (l, wikiArticle) else null
      ).filter(_ != null)
    ).groupByKey()
  ...
  def rankLangsReduceByKey(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] =
    rdd.flatMap(
      wikiArticle => langs.map(
        l => if (wikiArticle.mentionsLanguage(l)) (l, 1) else (l, 0)
      )
